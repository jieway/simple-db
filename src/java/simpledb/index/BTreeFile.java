...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// 更新右兄弟节点 B+ 树的兄弟节点从左到右是一条链表
		// 也就是把 newpage 插入到其中 newpage -> rightpage
		if (page.getRightSiblingId() != null) {
			BTreePageId rightId = page.getRightSiblingId();
			BTreeLeafPage rightPage = (BTreeLeafPage) getPage(tid, dirtypages, rightId, Permissions.READ_ONLY);
		...
		// 更新 page 和 newpage 之间的指针 ，实现了 page -> newpage -> rightpage
		...
		BTreeEntry entry = new BTreeEntry(next.getField(parentPage.keyField), page.getId(), newPage.getId());
		...
		// 返回插入 field 应当插入位置的 leaf page
		boolean isRight = field.compare(Op.GREATER_THAN_OR_EQ, next.getField(parentPage.keyField));
	 ...
	 * which of the two pages to return.f
	 * @see #getParentWithEmptySlots(TransactionId, Map, BTreePageId, Field)
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		List<BTreeEntry> movedTuples = new ArrayList<>();
		...
		dirtypages.put(parentPage.getId(), parentPage);
		boolean isRight = field.compare(Op.GREATER_THAN_OR_EQ, mid.getKey());
		return isRight ? newPage : page;
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// isRightSibling = true 表示 [page sibling] 反之表示 [sibling page]
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// the left internal page right
		BTreeEntry right = it.next();
		leftSibling.deleteKeyAndRightChild(right);
		// the right internal right page
		BTreeEntry left = page.iterator().next();
		BTreeEntry entry = new BTreeEntry(parentEntry.getKey(), right.getRightChild(), left.getLeftChild());
		page.insertEntry(entry);
		page.insertEntry(right);

		// Moved entry from sibling to curPage
		int curTuples = page.getNumEntries();
		while (curTuples < moved && it.hasNext()) {
